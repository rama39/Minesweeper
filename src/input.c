...
int receive_command(Board *board) {
    int x, y;
    ...
    if(buffer[0] == 'X') {
        if(sscanf(buffer+1, " %i %i", &x, &y) != 2) return PLAY;
        if(!(y >= 0 && y < board->h && x >= 0 && x < board->w)) return PLAY;
        if(board->_[y][x].revealed) return PLAY;
        if(board->_[y][x].flagged) board->_[y][x].flagged = 0, board->flag_count--;
        else board->_[y][x].flagged = 1, board->flag_count++;
        return PLAY;
    } else {
        if(sscanf(buffer, " %i %i", &x, &y) != 2) return PLAY;
        if(!(y >= 0 && y < board->h && x >= 0 && x < board->w)) return PLAY;
        if(board->_[y][x].flagged) return PLAY;
...
void handle_victory(Board *board, int length) {
    printf("You took %i seconds!\n", length);
    FILE * wins = fopen("wins.txt", "a");
    if(wins != NULL) 
        fprintf(wins, "(%i, %i): %is\n", board->w, board->h, length);
    else
        printf("Save file could not be opened or created\n");
}